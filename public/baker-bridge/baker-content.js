(() => {
  ...
  const VERSION = '0.1.0';
  const post = (type, detail = {}) => window.postMessage({ source: EXT_SOURCE, type, version: VERSION, ...detail }, window.location.origin);
  const announceReady = () => post('BAKER_BRIDGE_EXTENSION_READY');

  window.addEventListener('message', async (event) => {
    if (event.source !== window || event.origin !== window.location.origin) return;
    const data = event.data;
    if (!data || data.source !== WEB_SOURCE) return;
    if (data.type === 'BAKER_BRIDGE_PING') return announceReady();
