...
async function extractRipleyHours() {
  ...
  function showOverlay(title, detail, state = 'working') {
    ...
    overlay.innerHTML = `
      <div style="width:min(540px,94vw);background:#fffaf7;border:1px solid #eadfd9;border-radius:30px;padding:36px;box-shadow:0 30px 100px rgba(0,0,0,.3);text-align:center">
        <div style="width:62px;height:62px;border-radius:20px;background:#fff0f3;color:${accent};display:flex;align-items:center;justify-content:center;margin:0 auto 18px;font-size:29px">✦</div>
        <div style="font-size:12px;letter-spacing:.14em;text-transform:uppercase;font-weight:800;color:#e85d70;margin-bottom:8px">Fieldwork by Baker</div>
        <div style="font-family:Georgia,serif;font-size:29px;font-weight:700;color:#332c28;margin-bottom:11px">${escapeHtml(title)}</div>
        <div style="font-size:14px;line-height:1.65;color:#76675f">${escapeHtml(detail)}</div>
        ${working ? '<div style="width:36px;height:36px;border:4px solid #f0dfdf;border-top-color:#e85d70;border-radius:50%;margin:25px auto 0;animation:bakerSpin .8s linear infinite"></div>' : '<div style="margin-top:22px;font-size:13px;font-weight:700;color:#5f514a">You can return to Fieldwork by Baker.</div>'}
      </div>
      <style>@keyframes bakerSpin{to{transform:rotate(360deg)}}</style>
    `;
  }
  ...
  try {
    ...
    const looksLikeHours = /(total\s*hours|hours?\s*history|fieldwork\s*hours|hour\s*entries|fieldwork\s*history)/i.test(heading)
      || (/date/.test(tableText) && /hour/.test(tableText));
    ...
    if (!looksLikeHours) {
      ...
      return {
        ok: false,
        code: 'NOT_TOTAL_HOURS',
        message: 'Ripley is open, but it is not on the Total Hours / Hours History page yet.',
      };
    }
    ...
    const pageLinks = [...document.querySelectorAll('a[href]')]
      .map((anchor) => {
        try {
          return { url: new URL(anchor.href, location.href), label: (anchor.textContent || '').trim() };
        } catch {
          return null;
        }
      })
      .filter((item) => item
        ...
        && item.url.pathname === currentPath
        && (/(page|paged|offset|start)=/i.test(item.url.search) || /^(next|previous|prev|\d+|›|»|‹|«)$/i.test(item.label)))
      .slice(0, 24);
    ...
    for (const item of pageLinks) {
      ...
      try {
        ...
        // Skip a page that cannot be read; current page data is still preserved.
      }
    }
    ...
    return {
      ok: false,
      code: 'RIPLEY_READ_ERROR',
      message: error instanceof Error ? error.message : 'Baker could not read the Ripley hours page.',
    };
  }
}
...
async function startTransfer() {
  ...
  if (!tab?.id) {
    return {
      ok: false,
      code: 'NO_RIPLEY_TAB',
      message: 'No open Ripley tab was found. Click Open Ripley, sign in, and navigate to Total Hours first.',
    };
  }
  ...
  try {
    const results = await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      world: 'MAIN',
      func: extractRipleyHours,
    });
    return results?.[0]?.result || {
      ok: false,
      code: 'NO_RESULT',
      message: 'Baker Bridge did not receive data from the Ripley tab.',
    };
  } catch (error) {
    return {
      ok: false,
      code: 'SCRIPT_ERROR',
      message: error instanceof Error ? error.message : 'Baker Bridge could not access the Ripley tab.',
    };
  }
}
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  startTransfer().then(sendResponse).catch((error) => sendResponse({
    ok: false,
    code: 'EXTENSION_ERROR',
    message: error instanceof Error ? error.message : 'Baker Bridge failed.',
  }));
  return true;
});
