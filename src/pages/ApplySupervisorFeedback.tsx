...
type FeedbackSession = {
  ...
  reviewEntry: { id: string; date: string; duration: number } | null;
  feedback: {
    entryId: string;
    ...
    <div className="min-h-[80vh] bg-[#FFFCF9] py-10 px-4">
      <div className="max-w-2xl mx-auto bg-white rounded-3xl border border-[#F2EDEA] p-7 lg:p-9 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          ...
          <div className="rounded-xl bg-[#FAF8F6] p-4"><div className="text-xs text-[#A8998E]">Entry</div><div className="font-medium text-[#332C28] mt-1">{session.reviewEntry ? `${session.reviewEntry.date} • ${session.reviewEntry.duration} hrs` : session.feedback.entryId}</div></div>
        </div>
        ...
        {applied ? (
          <div className="rounded-2xl bg-[#E8F5EE] p-5 text-[#4B8C69] flex items-start gap-3"><CheckCircle2 size={20} className="mt-0.5" /><div><div className="font-semibold">Feedback applied</div><div className="text-sm mt-1">The entry status and supervisor note were updated in this browser.</div><Link to="/dashboard" className="inline-block text-sm underline mt-3">Return to dashboard</Link></div></div>
        ) : canApply ? (
