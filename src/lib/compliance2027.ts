...
function buildMonthlyCompliance(month: string, entries: HourEntry[]): MonthlyCompliance {
  ...
  const individualSupervisionMinutes = entries.reduce(
    (sum, entry) => sum + (entry.individualSupervisionMinutes ?? entry.supervisionMinutes ?? 0),
    0
  );
  ...
  const observationState: ComplianceState = totalHours === 0
    ...
    : observationMinutes >= requiredObservationMinutes
      ? 'pass'
      : 'fail';
  const individualState: ComplianceState = supervisionMinutes === 0
    ? 'unknown'
  ...
  const checks: ComplianceCheck[] = [
    ...
    },
    {
      ...
      value: supervisionMinutes > 0 ? formatPercent(individualSupervisionMinutes / supervisionMinutes) : 'Not recorded',
      requirement: 'At least 50% of supervised hours individual',
      detail: 'Group supervision may not exceed 50% of supervised hours.',
    },
  ];
...
export function evaluateCompliance(entries: HourEntry[]): ComplianceSummary {
  ...
  const unrestrictedHours = entries
    .filter((entry) => entry.activityCategory === 'UNRESTRICTED')
    .reduce((sum, entry) => sum + entry.duration, 0);
  const restrictedHours = Math.max(0, actualHours - unrestrictedHours);
  const unrestrictedRatio = actualHours > 0 ? unrestrictedHours / actualHours : 0;
  ...
  const globalChecks: ComplianceCheck[] = [
    {
      ...
      state: actualHours === 0 ? 'unknown' : unrestrictedRatio >= BACB_2027_RULES.minUnrestrictedRatio ? 'pass' : 'fail',
      value: actualHours === 0 ? 'No hours yet' : formatPercent(unrestrictedRatio),
      requirement: 'At least 60% unrestricted',
      detail: 'The 2027 BCBA requirements require at least 60% of total supervised fieldwork hours to be unrestricted.',
    },
    {
